...
When $k_{eff}$ exceeds 1, the chain reaction blows up and the system is said to have reached criticality. To maintain safety, a safety threshold is defined near $k_{eff}$ = 1. So, ideally a neutron source should exhibit a $k_{eff}$ value at its safety threshold. The Monte Carlo N-Particle (MCNP) code uses Monte Carlo simulations to estimate $k_{eff}$ for a given geometry and materials.
...
Maybe I can put my txt files? If not, I'm having trouble structuring this section.
